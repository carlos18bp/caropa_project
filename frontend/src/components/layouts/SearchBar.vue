      ...
      <div class="relative z-50 bg-white shadow-lg w-full p-6">
        ...
        <div class="mt-4">
          <div class="grid grid-cols-3 gap-4">
            <!-- Suggestion items -->
            <RouterLink v-if="products" :to="{ name: 'product', params: { product_ref: product.ref } }" v-for="product in products" :key="products.id" class="flex items-center">
              <img :src="product.gallery_urls[0]" alt="Product image" class="w-16 h-16 object-cover rounded"/>
              <div class="ml-4">
          ...
          <div class="mt-4 text-right">
            <RouterLink :to="{ name: 'catalog' }" class="font-medium me-6">See all products</RouterLink>
          </div>
        </div>
      </div>
    </div>
