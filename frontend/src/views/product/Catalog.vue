                ...
                <div class="mt-6 grid gap-x-6 gap-y-10 md:grid-cols-2 xl:grid-cols-3">
                  <div 
                    ...
                    <div class="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 aspect-none group-hover:opacity-75 h-80 mb-4">
                      <img :src="product.gallery_urls[0]" class="object-cover object-center h-full w-full" />
                    </div>
                    <div class="mt-2 flex justify-between">
                      <RouterLink :to="{ name: 'product', params: { product_ref: product.ref } }">
                        <span aria-hidden="true" class="absolute inset-0" />
                        <h3 class="text-sm">{{ product.product_detail.name }}</h3>
                        <p class="text-sm font-medium text-gray-700 text-left mb-2">{{ product.product_detail.description }}</p>
                        <p class="text-sm font-medium text-gray-900 text-left">$ {{ product.product_detail.price }}</p>
                      </RouterLink>
                    </div>
                  </div>
                </div>
...
<script setup>
  ...
  /**
   * Initializes the product store and defines computed properties and refs.
   */
