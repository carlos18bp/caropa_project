...
export const useProductStore = defineStore('productStore', {
  ...
  getters: {
    ...
    uniqueProductsByPrimaryCategory: (state) => (primaryCategory) => {
      const productsMap = new Map();
      state.products.forEach((product) => {
        if (product.categories.some(category => category.name === primaryCategory && category.is_primary)) {
          const ref = product.ref;
          const color = product.color.name;
          if (!productsMap.has(ref)) {
            productsMap.set(ref, {
    ...
    minPriceInFilteredProducts: (state) => {
      ...
      return Math.min(...state.filteredProductsByCategory.map(product => parseFloat(product.product_detail.price)));
    },
    /**
    ...
    maxPriceInFilteredProducts: (state) => {
      if (state.filteredProductsByCategory.length === 0) return 0;
      return Math.max(...state.filteredProductsByCategory.map(product => parseFloat(product.product_detail.price)));
    },
    /**
    ...
    totalCartPrice: (state) => {
      return state.cartProducts.reduce((total, product) => {
        return total + (parseFloat(product.product_detail.price) * product.quantity);
      }, 0);
    }
  },
  actions: {
    ...
    async fetchProducts() {
      if (!this.dataLoaded) {
        try {
          const data = await get_request('products-data/');
    ...
    filterProductsByCategory() {
      if (this.primaryCategorySeleted === 'All') {
        ...
        this.filteredProductsByCategory = this.uniqueProductsByPrimaryCategory(this.primaryCategorySeleted);
      }
     ...
     */
    filterProducts({ nonPrimaryCategoriesSelected = [], 
      ...
      if (JSON.stringify(this.nonPrimaryCategoriesSelected) !== JSON.stringify(nonPrimaryCategoriesSelected)) {
        if (is_nonPrimaryCategoriesFilter) this.nonPrimaryCategoriesSelected = nonPrimaryCategoriesSelected;
      }
      if (this.minPrice !== minPrice && is_priceFilter) { this.minPrice = minPrice }
      if (this.maxPrice !== maxPrice && is_priceFilter) { this.maxPrice = maxPrice }
      if (JSON.stringify(this.colorsSelected) !== JSON.stringify(colorsSelected)) {
        if (is_colorsFilter) this.colorsSelected = colorsSelected;
      }

      this.filterProductsByNonPrimaryCategory(this.nonPrimaryCategoriesSelected);
    ...
    filterProductsByNonPrimaryCategory(nonPrimaryCategoriesSelected) {
      ...
      this.filteredProducts.forEach(product => {
        if (product.categories.some(category => nonPrimaryCategoriesSelected.includes(category.name) && !category.is_primary)) {
    ...
    addProductToCart(addProduct) {
      const existingProduct = this.cartProducts.find(product => product.id === addProduct.id);
    ...
    removeProductFromCart(removeProductId) {
      const removeProduct = this.cartProducts.find(product => product.id === removeProductId);
      
      if (removeProduct.quantity > 1) {
        removeProduct.quantity -= 1;
      } else {
        this.cartProducts = this.cartProducts.filter((product) => product !== removeProduct);
      }
    },
  },
});
